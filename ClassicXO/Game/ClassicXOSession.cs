    ...
    public class ClassicXOSession : IXOSession
    {
        ...
        private void CreateField()
        {
            ...
            for (int i = 0; i < FieldConstants.FieldWidth; i++)
            {
                ...
                {
                    _field[i, j] = new Cell(i, j, State.Empty);
                }
            }
        }
        ...
        {
            var victoryCombination = GetVictoryCombinationCells(State.Computer);
        ...
        {
            var victoryCombination = GetVictoryCombinationCells(State.Human);
            if (victoryCombination != null)
            {
                var victoryInfo = new VictoryInformation()
                {
        ...
        private IEnumerable<Cell>? GetVictoryCombinationCells(State subject)
        {
            for (int i = 0; i < 3; i++)
            {
